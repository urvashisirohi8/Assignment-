    ...
    html.Div([#TASK 2.2: Add two dropdown menus
        ...
        dcc.Dropdown(
            ...
            options=[{'label':'Yearly Statistics','value':'Yearly Statistics'},
                    {'label':'Recession Yearly Statistics','value':'Recession Yearly Statistics'}],
...
#Plot 2 Calculate the average number of vehicles sold by vehicle type       
        ...
        R_chart2  = dcc.Graph(figure=px.bar(average_sales,x='Vehicle_Type',y='Automobile_Sales',title='Average Automobile Sales as per vehicle types'))        
# Plot 3 Pie chart for total expenditure share by vehicle type during recessions
        # use groupby to create relevant data for plotting
        exp_rec= recession_data.groupby('Vehicle_Type')['Advertising_Expenditure'].sum().reset_index()
        R_chart3 = dcc.Graph(figure=px.pie(exp_rec,value='Advertising_Expenditure',names='Vehicle_Type',title='Total expenditure shared by vehicle type during recession'))

# Plot 4 bar chart for the effect of unemployment rate on vehicle type and sales
        unemp_data= recession_data.groupby(['Vehicle_Type','unemployment_rate'])['Automobile_Sales'].mean().reset_index()
        R_chart4 = dcc.Graph(figure=px.bar(unemp_data,x='unemployment_rate',y='Automobil_Sales',title='Effect of unemployment on vehicle sales') ) 
...
# TASK 2.6: Create and display graphs for Yearly Report Statistics
 # Yearly Statistic Report Plots                             
    elif (input_year and selected_statistics=='Yearly_Statistic') :
...
# Plot 2 Total Monthly Automobile sales using line chart.
        ...
        Y_chart4 = dcc.Graph(figure=px.pie(exp_data, values="Advertising_Expenditure", names="Vehicle_Type",title='Advertisement Expenditure per Vehicle type in the year {}'.format(input_year)))
